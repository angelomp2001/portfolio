...
df_2 = pd.read_csv(r'C:\Users\Angelo\Documents\vscode\portfolio\portfolio-1\sprint 9 geo_data_1.csv')
df_3 = pd.read_csv(r'C:\Users\Angelo\Documents\vscode\portfolio\portfolio-1\sprint 9 geo_data_2.csv')
...
# we drop duplicates and ['id'] as we don't need it for anything
...
def product_predictions(
        ...
        '''
            Train a model on the training set and make predictions on the validation set.
        '''
...
def stats(predictions: pd.Series, wells_to_select=WELLS_TO_SELECT) -> pd.Series:
    '''
...
def bootstrap_predictions(
    ...
    for sample in range(samples):
        ...
        bootstrap_sample = validation.sample(n=len(validation), replace=True, random_state=sample_seed)
        preds = model.predict(bootstrap_sample[features])
        bootstrap_sample['predictions'] = preds
...
for region, df in dfs.items():
    ...
    predictions = product_predictions(training=training, validation=validation, model=model)
    #  2.2. Train the model and make predictions for the validation set.
    # define features and target
    # target = df['product'].name
    # features = df.drop(columns=target).columns
    #see product_predictions)
    ...
    stats(predictions=predictions, wells_to_select=WELLS_TO_SELECT)
    #  2.4. Print the average volume of predicted reserves and model RMSE.
...
#  4.3. Provide findings: suggest a region for oil wells' development and justify the choice. Calculate the profit for the obtained volume of reserves.
    ...
    top_200_total_profit = bootstrap_predictions(validation, model, features, samples=POINTS_STUDIED, random_state=random_state)
