...
OUTPUT_IMAGE: str | None = "docs/DAG.png"
...
# Styling: Intra-Module Edge
INTRA_EDGE_COLOR: str = "purple" # color of intra-module edges
INTRA_EDGE_ALPHA: float = 0.5 # alpha of intra-module edges
INTRA_EDGE_MUTATION_SCALE: int = 10 # arrow head size of intra-module edges
...
class DependencyScanner(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        ...
        # When module="src.data_preprocessing", add edge directly to it
        if node.module in self.project_modules:
            self._add_import_edge(node.module)
    ...
    def visit_Constant(self, node: ast.Constant):
        if isinstance(node.value, str):
            ...
            if any(val.endswith(ext) for ext in [".csv", ".pkl", ".json", ".h5", ".joblib", ".npy", ".txt", ".db", ".sqlite", ".png", ".jpg", ".jpeg"]):
                self._add_import_edge("Other")
        self.generic_visit(node)
...
class UsagesScanner(ast.NodeVisitor):
    def __init__(self):
        self.used_names = set()
...
class ModuleContentScanner(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        scanner = UsagesScanner()
        for stmt in node.body:
            scanner.visit(stmt)
        self.usages[name] = list(scanner.used_names)
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        for stmt in node.body:
            scanner.visit(stmt)
        self.usages[f"{node.name}()"] = list(scanner.used_names)
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        if node.module in self.project_modules:
            for alias in node.names:
                if alias.name == "*":
                    # For wildcard imports, record the module it imported from
    ...
    def visit_Constant(self, node: ast.Constant):
        if isinstance(node.value, str):
            val = node.value.lower()
            if any(val.endswith(ext) for ext in [".csv", ".pkl", ".json", ".h5", ".joblib", ".npy", ".txt", ".db", ".sqlite", ".png", ".jpg", ".jpeg"]):
                # Record this as an import from "Other" to establish the child edge later
                if node.value not in self.imports:
                    # To make it easily parseable in visualize_module_dependencies
...
def build_module_graph(project_root: Path):
    ...
    module_usages: dict[str, dict[str, list[str]]] = {}
    ...
    for file_path, module_name in project_modules.items():
        ...
        module_usages[module_name] = api_analyzer.usages
        
        # separate data files from standard imports
        standard_imports = []
        for i in api_analyzer.imports:
            i_lower = i.lower()
            if any(i_lower.endswith(ext) for ext in [".csv", ".pkl", ".json", ".h5", ".joblib", ".npy", ".txt", ".db", ".sqlite", ".png", ".jpg", ".jpeg"]):
                other_files.add((module_name, i))
            else:
                standard_imports.append(i)
                
        module_imports[module_name] = standard_imports
        
    if other_files:
        graph.add_node("Other", kind="module")
        module_classes["Other"] = []
        module_functions["Other"] = []
        module_variables["Other"] = sorted(list({f for _, f in other_files}))
        module_imports["Other"] = []
        for mod, f in other_files:
            graph.add_edge(mod, "Other", kind="import")
            module_imports[mod].append(f) # add back as a standard import for drawing lines
    ...
    for mod_name in module_imports:
        ...
        for i in module_imports[mod_name]:
            ...
            if i.endswith(" *"):
                 formatted_imports.append(i)
            elif i in all_function_names:
                formatted_imports.append(f"{i}()")
            else:
                 formatted_imports.append(i)
        module_imports[mod_name] = formatted_imports




    return graph, module_classes, module_functions, module_variables, module_imports, module_usages
...
def visualize_module_dependencies(
    ...
    def get_module_children(m: str) -> list[str]:
        return module_classes.get(m, []) + module_functions.get(m, []) + module_variables.get(m, [])

    modules = [
        n for n, d in graph.nodes(data=True)
        if d.get("kind") == "module" and len(get_module_children(n)) > 0
    ]
    ...
    # First, sort alphabetically to get a stable base ordering
    for lvl in layers:
        layers[lvl].sort()
    ...
    def get_forward_positions(p, n, order_p):
        p_base = order_p
        p_children = get_module_children(p)
        n_exports = {strip_prefix(exp).replace("()", "") for exp in get_module_children(n)}
        positions = []
        for caller, used_names in module_usages.get(p, {}).items():
            if caller in p_children:
                used_base = [name.replace("()", "") for name in used_names]
                if any(imp in used_base for imp in n_exports):
                    positions.append(p_base + (p_children.index(caller) + 1) / (len(p_children) + 1))
        return positions if positions else [p_base + 0.5]

    def get_backward_positions(n, s, order_s):
        s_base = order_s
        s_children = get_module_children(s)
        s_exports = {strip_prefix(exp).replace("()", ""): exp for exp in s_children}
        positions = []
        for caller, used_names in module_usages.get(n, {}).items():
            used_base = [name.replace("()", "") for name in used_names]
            for imp in used_base:
                if imp in s_exports:
                    exp_full = s_exports[imp]
                    positions.append(s_base + (s_children.index(exp_full) + 1) / (len(s_children) + 1))
        return positions if positions else [s_base + 0.5]

    max_bary_iterations = 10
    for _ in range(max_bary_iterations):
        changed = False
        
        # Forward sweep
        for lvl in sorted(layers.keys())[1:]:
            prev_order = {n: i for i, n in enumerate(layers[lvl-1])}
            bary = {}
            for n in layers[lvl]:
                preds = [p for p in dep_graph.predecessors(n) if p in prev_order]
                if preds:
                    all_pos = []
                    for p in preds:
                        all_pos.extend(get_forward_positions(p, n, prev_order[p]))
                    bary[n] = sum(all_pos) / len(all_pos) if all_pos else 0
                else:
                    bary[n] = (layers[lvl].index(n) / len(layers[lvl])) * max(1, len(prev_order))
            
            new_order = sorted(layers[lvl], key=lambda x: (bary[x], layers[lvl].index(x)))
            if new_order != layers[lvl]:
                layers[lvl] = new_order
                changed = True
                
        # Backward sweep
        sorted_lvls = sorted(layers.keys())
        for lvl in sorted_lvls[-2::-1]:
            next_order = {n: i for i, n in enumerate(layers[lvl+1])}
            bary = {}
            for n in layers[lvl]:
                succs = [s for s in dep_graph.successors(n) if s in next_order]
                if succs:
                    all_pos = []
                    for s in succs:
                        all_pos.extend(get_backward_positions(n, s, next_order[s]))
                    bary[n] = sum(all_pos) / len(all_pos) if all_pos else 0
                else:
                    bary[n] = (layers[lvl].index(n) / len(layers[lvl])) * max(1, len(next_order))
    ...
    # Precompute exact container heights and widths per module
    container_height: dict[str, float] = {}
    container_width: dict[str, float] = {}
    ...
    for m in modules:
        ...
        # The module name is bold and uses a larger font size, so it takes up more space per character
        effective_mod_length = len(mod_label) * 1.4
        max_len_str = max([effective_mod_length] + [len(c) for c in children]) if children else effective_mod_length
        # Scale: ~0.15 width natively per char, cap a minimum width. Increased by ~30% per user request.
        calc_width = max(CONTAINER_WIDTH, max_len_str * 0.26 + 2 * side_padding)
        container_width[m] = calc_width
    ...
    if provisional_center:
        min_x = min(cx - container_width[m] / 2.0 for m, (cx, _) in provisional_center.items())
    ...
    for m, (cx, cy) in provisional_center.items():
        mod_center[m] = (cx + shift_x, cy)    # ---- 4. Draw containers and child boxes ----
    ...
    for u, v in dep_edges:
        ...
        u_children = get_module_children(u)
        v_children = get_module_children(v)
        ...
        if u in module_imports:
            for imp in module_imports[u]:
                if imp.endswith(" *"):
                     origin_mod = imp.replace(" *", "")
                     if origin_mod == v:
                         pass # fallback to module-module edge
               
                else:
                     raw_imp = strip_prefix(imp)
                     
                     if v == "Other" and any(raw_imp.lower().endswith(ext) for ext in [".csv", ".pkl", ".json", ".h5", ".joblib", ".npy", ".txt", ".db", ".sqlite", ".png", ".jpg", ".jpeg"]):
                         if raw_imp in child_box_coords.get(v, {}):
                             v_left, v_right, v_y = child_box_coords[v][raw_imp]
                             start_x = cx_u + container_width[u] / 2.0
                             start_y = cy_u
                     ...
                     if raw_imp in exports_map:
                         for export_m, export_full in exports_map[raw_imp]:
                             if export_m == v:
                                 used_by = []
                                 for caller, used_names in module_usages.get(u, {}).items():
                                     if caller in u_children:
                                         base_imp = raw_imp.replace("()", "")
                                         if base_imp in used_names:
                                             used_by.append(caller)
                                 
                                 if used_by:
                                     for caller in used_by:
                                         u_left, u_right, u_y = child_box_coords[u][caller]
                                     ...
                                     if export_full in child_box_coords.get(v, {}):
                                         v_left, v_right, v_y = child_box_coords[v][export_full]
                                         start_x = cx_u + container_width[u] / 2.0
                                         start_y = cy_u
                                         arrow = patches.FancyArrowPatch(
        ...
        if not child_edge_drawn:
            ...
            end_x = cx_v - container_width[v] / 2.0
            end_y = cy_v + h_v / 2.0
    ...
    # ---- 6. Axes limits & title ----
    if mod_center:
        min_x = min(cx - container_width[m] / 2.0 for m, (cx, cy) in mod_center.items())
        max_x = max(cx + container_width[m] / 2.0 for m, (cx, cy) in mod_center.items())
...
def run():
    ...
    graph, module_classes, module_functions, module_variables, module_imports, module_usages = build_module_graph(project_root)
    print(f"Modules: {graph.number_of_nodes()}, Module edges: {len([e for e in graph.edges(data=True) if e[2].get('kind') == 'import'])}")




    output_path = (project_root / OUTPUT_IMAGE).resolve() if OUTPUT_IMAGE else None
    visualize_module_dependencies(graph, module_classes, module_functions, module_variables, module_imports, module_usages, output=output_path)
